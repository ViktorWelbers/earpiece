...
def env(monkeypatch, tmp_path):
    ...
    for var in ("AGENT_CWD", "AGENT_AUTO_TOOLS", "STT_BASE_URL", "STT_MODEL", "STT_API_KEY"):
...
def test_minimal_env(env):
    ...
    assert s.agent_cmd == "fake-agent --acp"
...
def test_env_wins_over_config_file(env, tmp_path):
    write_config(env, tmp_path, 'AGENT_CMD = "from-file"\n')
    env.setenv("AGENT_CMD", "from-env")
    s = Settings.from_env("mission")
    assert s.agent_cmd == "from-env"
...
def test_save_config_roundtrip(env, tmp_path):
    ...
    env.delenv("AGENT_CMD")
    save_config({"AGENT_CMD": 'weird "quoted" cmd', "AGENT_AUTO_TOOLS": "jira_*"})
    s = Settings.from_env("mission")
    assert s.agent_cmd == 'weird "quoted" cmd'  # TOML escaping survives the roundtrip
    assert s.agent_auto_tools == "jira_*"
